    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="收款记录" name="receipts">
        ...
        >
          ...
          <el-table-column prop="receiptNo" label="收据号" min-width="130" />
          <el-table-column label="金额" width="120" align="right">
            <template #default="{ row }">¥{{ (row.amount || 0).toLocaleString() }}</template>
          </el-table-column>
          <el-table-column label="关联合同" min-width="160">
            <template #default="{ row }">{{ row.contractNo || '-' }}</template>
          </el-table-column>
          <el-table-column prop="paymentChannelName" label="支付通道" min-width="80" />
          <el-table-column prop="receivedDate" label="收款日期" width="100" />
          <el-table-column label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="statusType(row.status)" size="small" style="text-align:center;">
...
.receipt-table :deep(.el-table__body-wrapper) {
  overflow-x: auto;
}
.receipt-table :deep(.cell) {
