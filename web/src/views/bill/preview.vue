...
/** Load both bill data and PDF blob */
async function loadPreview() {
  ...
  try {
    const [res, blob] = await Promise.all([
      getDebitNote(id),
      exportDebitNotePdf(id)
    ])

    // Map bill metadata (for the action bar)
    bill.value = {
      id: res.id,
      billNo: res.billNo || '',
      generatedAt: (res.createdAt || '').slice(0, 10),
      isHistorical: res.isHistorical || false,
    }

    // Keep the blob for download and create preview URL
    pdfBlob.value = blob
...
function downloadPdf() {
  ...
  const filename = `${bill.value?.billNo || 'bill'}.pdf`
