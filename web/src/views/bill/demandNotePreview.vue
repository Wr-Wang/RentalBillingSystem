<template>
  <div class="demandnote-preview">
    ...
    <div class="loading-state" v-else-if="pdfLoading">
      ...
      <span>正在生成 DemandNote PDF 预览...</span>
    </div>
...
import { demandNotePreview } from '../../api/index'
...
/** Generate PDF blob URL and trigger browser download */
...
async function loadPreview() {
  ...
  try {
    const blob = await demandNotePreview()
    pdfBlob.value = blob
    pdfUrl.value = window.URL.createObjectURL(blob)
    pdfLoading.value = false
  } catch {
    ElMessage.error('加载 DemandNote PDF 失败')
    pdfLoading.value = false
  }
}
...
function downloadPdf() {
  ...
  const filename = `DemandNote_${generatedAt.value}.pdf`
  triggerDownload(pdfBlob.value, filename)
  ElMessage.success('DemandNote PDF 已下载')
}
