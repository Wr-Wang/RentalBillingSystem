...
public class DapperCompanyRepository : ICompanyRepository
{
    ...
    public DapperCompanyRepository(IDbConnectionFactory db, ISqlLoader sql, IAuditLogWriter auditWriter, IChangeTracker? tracker = null, RepositoryAuditService? auditService = null)
    {
        _db = db; _sql = sql;
    ...
    public async Task<Company?> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        ...
        var e = await conn.QuerySingleOrDefaultAsync<Company>(_sql.Get("Organization.Select.Company.ById"), new { Id = id });
        if (e != null) _tracker?.Track(e, "Companies");
        return e;
    }
    ...
    {
        using var conn = _db.CreateConnection(); conn.Open();
        var list = (await conn.QueryAsync<Company>(_sql.Get("Organization.Select.Company.All"))).ToList();
        foreach (var e in list) _tracker?.Track(e, "Companies");
        return list;
    }
    ...
    public async Task DeleteAsync(Company entity, CancellationToken ct = default)
    {
        using var conn = _db.CreateConnection(); conn.Open();
        await conn.ExecuteAsync(_sql.Get("Organization.Delete.Company.ById"), new { entity.Id });
        var oldEntity = await GetByIdAsync(entity.Id, ct);
        if (oldEntity != null)
            await _auditService.WriteDeleteAsync("Companies", oldEntity, ct);
    }

    public async Task<bool> ExistsAsync(Guid id, CancellationToken ct = default)
    {
        using var conn = _db.CreateConnection(); conn.Open();
        return await conn.QuerySingleAsync<int>(_sql.Get("Organization.Select.Company.Exists"), new { Id = id }) > 0;
    }

    public async Task<Company?> GetByNameAsync(string name, CancellationToken ct = default)
    {
        using var conn = _db.CreateConnection(); conn.Open();
        return await conn.QuerySingleOrDefaultAsync<Company>(_sql.Get("Organization.Select.Company.ByName"), new { Name = name });
    }

    public async Task<List<Company>> GetActiveAsync(CancellationToken ct = default)
    {
        using var conn = _db.CreateConnection(); conn.Open();
        return (await conn.QueryAsync<Company>(_sql.Get("Organization.Select.Company.Active"))).ToList();
    }
