...
public partial class HousingUnitService : IHousingUnitService
{
    ...
    {
        var u = await _uow.HousingUnits.GetByIdAsync(id, ct) ?? throw new KeyNotFoundException("房源不存在");
        await _uow.HousingUnits.DeleteAsync(u, ct);
        await _uow.CommitAsync(ct);
    }
    ...
    public async Task<List<TreeBuildingDto>> GetTreeAsync(Guid companyId, CancellationToken ct = default)
    {
        ...
        return units.GroupBy(u => u.BuildingName).Select(g => new TreeBuildingDto
        {
            Id = "B:" + g.Key,
            Name = g.Key,
            Children = g.GroupBy(u => u.FloorName).Select(fg => new TreeFloorDto
            {
                Id = "F:" + fg.Key,
                Name = fg.Key,
                Children = fg.Select(u => new TreeRoomDto { Id = u.Id.ToString(), Name = u.FullCode ?? u.UnitNo }).ToList()
            }).ToList()
        }).ToList();
    }
    private async Task<List<HousingUnitResponse>> MapAsync(IEnumerable<HousingUnit> units, CancellationToken ct)
    {
        ...
        return units.Select(u => new HousingUnitResponse
        {
            ...
            FloorSortOrder = u.FloorSortOrder,
            UnitNo = u.UnitNo,
            FullCode = u.FullCode,
            RoomTypeId = u.RoomTypeId,
            RoomTypeName = u.RoomTypeId.HasValue ? types.GetValueOrDefault(u.RoomTypeId.Value) : null,
            Status = u.Status.Code,
            Area = u.Area,
            Orientation = u.Orientation,
            BaseRentAmount = u.BaseRentAmount,
